...
#include "indicator_control_model.h"   /* Model header file */
...
void rt_OneStep(void)
{
  ...
  /* Check for overrun */
  if (OverrunFlag) {
    rtmSetErrorStatus(indicator_control_model_M, "Overrun");
    return;
  }
...
int_T main(int_T argc, const char *argv[])
{
  ...
  fflush((NULL));
  while (rtmGetErrorStatus(indicator_control_model_M) == (NULL)) {
    /*  Perform application tasks here */
  }

  /* Terminate model */
  indicator_control_model_terminate();
  return 0;
}
